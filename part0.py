...
def normalize_string(s):
    ...
    ###
    print(len(s))
    j = ''.join([i for i in s.lower() if i.isalpha() or i.isspace()])
    print(len(j))
    return j
...
def update_pair_counts(pair_counts, itemset):
    ...
    ###
    for a,b in it.combinations(itemset,2):
        pair_counts[a,b]+=1
        pair_counts[b,a]+=1
    print(pair_counts)
    print(itemset)
    print(it.combinations(itemset,2))
    ###
...
def update_pair_counts1(pair_counts, itemset):
    """
    Updates a dictionary of pair counts for
    all pairs of items in a given itemset.
    """
...
update_pair_counts1(pair_counts, itemset_1)
assert len(pair_counts) == 6
update_pair_counts1(pair_counts, itemset_2)
...
def filter_rules_by_conf (pair_counts, item_counts, threshold):
    ...
    for key,val in pair_counts.items():
        if val/item_counts[key[0]] >= threshold:
            rules[(key[0],key[1])] = val/item_counts[key[0]]
...
def find_assoc_rules(receipts, threshold):
    ...
    for receipt in receipts:
        update_pair_counts1(pair_counts, receipt)
        update_item_counts(item_counts, receipt)
    result = filter_rules_by_conf(pair_counts, item_counts, threshold)
    return result
    ###
...
###
latin_rules=find_assoc_rules(make_itemsets(get_normalized_words(normalize_string(latin_text))),0.75)

###

# Inspect your result:
print_rules(type(latin_rules))
...
###
latin_rules=find_assoc_rules(make_itemsets(get_normalized_words(normalize_string(latin_text))),0.75)
english_rules=find_assoc_rules(make_itemsets(get_normalized_words(normalize_string(english_text))),0.75)
...
pair_counts=defaultdict(int)
item_counts=defaultdict(int)
for lists in groceries_file.split('\n'):
    for a,b in it.combinations(set(lists.split(',')),2):
        pair_counts[(a,b)] += 1
        pair_counts[(b,a)] += 1
    for a in set(lists.split(',')):
        item_counts[a] += 1
for key,val in pair_counts.items():
    if val/item_counts[key[0]]>= THRESHOLD and item_counts[key[0]] >= 10:
        basket_rules[key] = val/item_counts[key[0]]
